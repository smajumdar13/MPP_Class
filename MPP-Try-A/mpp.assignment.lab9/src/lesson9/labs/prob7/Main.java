...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		//System.out.println(intList.stream().sorted((i1,i2)->Math.abs(i1)-Math.abs(i2)).collect(Collectors.toList()));
		System.out.println(list.stream().sorted(Comparator.comparingInt(Math::abs)).collect(Collectors.toList()));
	...
	public static void ordering2(List<String> words) {
		...
		//System.out.println(words.stream().sorted((s1,s2)->new StringBuilder(s1).reverse().toString().compareTo(new StringBuilder(s2).reverse().toString())).collect(Collectors.toList()));
